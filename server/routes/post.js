...
router.post('/updatePost', async (req, res) => {
    ...
    db.run(
        ...
        function (err) {
            if (err) {
                console.error(err.message);
                res.status(500).json({ error: "Post update failed. Make sure you're the author of the post." });
            } else if (this.changes === 0) {
                console.error(err.message);
                res.status(500).json({ error: "Post not found or you don't have permission to update it." });
            } else {
  ...
  db.run(query, [postId, userId], function (err) {
    if (err) {
      ...
      res.status(500).json({ error: "Post deletion failed. Make sure you're the author of the post." });
        ...
        console.error(err.message);
        res.status(500).json({ error: "Post not found or you don't have permission to delete it." });
    } else {
