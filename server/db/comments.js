...
const newComment = { // Collected client side
...
function createComment(newComment) {
  ...
  db.run(
    ...
    [
      newComment.postId,
      newComment.commentsContent,
      newComment.commentsAuthor
    ],
    function (err) {
      if (err) {
...
function deleteComment(commentId, userId) {
  ...
  db.run(query, [commentId, userId], function (err) {
    if (err) {
      console.error(err.message);
      console.log("Comment deletion failed. Make sure you're the author of the comment.");
    } else if (this.changes === 0) {
      console.log("Comment not found or you don't have permission to delete it.");
    } else {
